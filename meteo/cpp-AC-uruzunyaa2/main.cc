...
int main(){
	...
	cin>>h>>w;
	//a[i][j]の確率で通れない。
	vector<vector<double>> a(w,vector<double>(h));
	rep(i,h)rep(j,w){
		cin>>a[j][i];
		a[j][i]/=100.0;
	}
	rep(i,w){
		rep(j,i)a[i].insert(a[i].begin(),1.0);
		rep(j,w-i-1)a[i].push_back(1.0);
	}
	...
	cout<<fixed<<setprecision(15)<<dp[h+w-2][1<<(w-1)]<<endl;
	return 0;
}
